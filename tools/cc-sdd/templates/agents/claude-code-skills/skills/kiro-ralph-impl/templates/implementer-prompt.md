# TDD Task Implementer (with Context Management)
...
> **Context check**: Run the `Context check command` provided in your startup context verbatim. Read the configured thresholds from `{{KIRO_DIR}}/settings/context-config.json` (or use the defaults: `threshold_percentage = 70%`, `warning_percentage = 60%`). Let `T%` be the threshold and `W%` be the warning percentage. Parse `Usage: X%` from the result. Output `[CTX: X%]`.
> If X% > T%: return PARTIAL_COMPLETION immediately (see Status Report format). If X% is between W% and T%: warn `[CTX: X% — wrapping up]` and continue only to a clean stopping point. If `unavailable`: log warning and continue normally.

### Step 2: Build Task Brief
Before writing any code, synthesize a concrete Task Brief from the spec sections you just read:

- **Acceptance criteria**: What observable behaviors must be true when done? Extract from the requirement sections. Be specific.
...
- **Design constraints**: What specific technical decisions from design.md must be followed?
...
> **Context check** (after building Task Brief): Run the `Context check command` verbatim. Let `T%` be the configured (or default 70%) threshold.
...
> **Context check** (after RED and GREEN phases): Run the `Context check command` verbatim. Let `T%` be the configured (or default 70%) threshold and `W%` be the configured (or default 60%) warning percentage.
...
> **Context check** (after VERIFY): Run the `Context check command` verbatim. Let `T%` be the configured (or default 70%) threshold.
...
- Verify that any newly introduced runtime-sensitive dependency or packaging assumption is reflected in validation or called out explicitly in `CONCERNS`
...
```
## Status Report
- STATUS: READY_FOR_REVIEW | BLOCKED | NEEDS_CONTEXT | PARTIAL_COMPLETION
- TASK: <task-id>
...
- BLOCKER_REMEDIATION: <only for BLOCKED -- what would unblock this?>
- MISSING: <only for NEEDS_CONTEXT -- describe exactly what additional context is needed>
- EVIDENCE: <concrete code paths, functions, and tests that prove the behavior>
- PROGRESS_SUMMARY: <only for PARTIAL_COMPLETION -- 1-3 sentences describing what code/tests were written>
- REMAINING_WORK: <only for PARTIAL_COMPLETION -- 1-3 sentences describing what still needs to be done>
```
