...
description: Implement approved tasks using TDD with native subagent dispatch, subtask/task selection, and context window management. Runs all pending tasks or selected tasks/subtasks.
...
## Role
You are the orchestrator for TDD implementation with context window management. You run in the main session context and dispatch executor, reviewer, and debugger subagents using the `Agent` tool.
...
- **BLOCKED** → dispatch debug subagent (see section below)
